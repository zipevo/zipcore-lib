...
CoinbasePayload.prototype.validate = function() {
  Preconditions.checkArgument(isUnsignedInteger(this.nVersion), 'Expect nVersion to be an unsigned integer');
  Preconditions.checkArgument(isUnsignedInteger(this.height), 'Expect nVersion to be an unsigned integer');
...
CoinbasePayload.prototype.toBuffer = function toBuffer() {
  ...
  payloadBufferWriter
    .writeUInt16LE(this.nVersion)
    .writeUInt32LE(this.height)
    .write(Buffer.from(this.regTxId, 'hex'));
