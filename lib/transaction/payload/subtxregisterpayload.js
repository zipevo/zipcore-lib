...
SubTxRegisterPayload.validatePayloadJSON = function (blockchainUserPayload) {
  ...
  var isZeroSignature = blockchainUserPayload.vchSig === constants.EMPTY_SIGNATURE;
  ...
  Preconditions.checkArgument(blockchainUserPayload.vchSig.length === constants.COMPACT_SIGNATURE_SIZE * 2 || isZeroSignature, 'Invalid vchSig size');
};
...
SubTxRegisterPayload.prototype.toJSON = function toJSON(options) {
  ...
  var payloadJSON = {
    ...
    vchSig: skipSignature ? '00' : this.vchSig
  };
  SubTxRegisterPayload.validatePayloadJSON(payloadJSON);
  return payloadJSON;
};
